...
class PPOAgent:
    ...
    def __init__(self, env_name, model_name=""):
        ...
        self.EPISODES = 100000 # total episodes to train through all environments
    ...
    def replay(self, states, actions, rewards, dones, next_states, logp_ts):
        ...
        #     pylab.legend(["advantages", "target"])

        #     pylab.savefig(self.env_name+"_"+str(self.episode)+".png")
        #     print("figure saved as", self.env_name+"_"+str(self.episode)+".png")
        #     pylab.clf()
    ...
    pylab.subplots_adjust(left=0.05, right=0.98, top=0.96, bottom=0.06)
    def PlotModel(self, worker_id, score, episode, save=True):
        self.scores_.append(score)
        self.episodes_.append(episode)
        self.average_.append(sum(self.scores_[-50:]) / len(self.scores_[-50:]))
        if str(episode)[-2:] == "00": #[-1:] == "0":# much faster than episode % 100
            pylab.plot(self.episodes_, self.scores_, 'b')
            pylab.plot(self.episodes_, self.average_, 'r')
            pylab.ylabel('Score', fontsize=18)
            pylab.xlabel('Episode', fontsize=18)
            # pylab.legend(["Score", "Average Score"])
            try:
                pylab.grid(True)
                pylab.savefig(self.env_name+"_"+str(self.episode)+"_"+str(worker_id)+".png")
            except OSError:
                pass
        # saving best models
    ...
    def run_batch(self):
        ...
        while True:
            ...
            for t in range(self.Training_batch):
                ...
                if done:
                    self.episode += 1
                    average, SAVING = self.PlotModel(0, score, self.episode)
    ...
    def run_multiprocesses(self, num_worker = 4):
        ...
        while self.episode < self.EPISODES:
            ...
            for worker_id, parent_conn in enumerate(parent_conns):
                ...
                if done:
                    average, SAVING = self.PlotModel(worker_id, score[worker_id], self.episode)
    ...
    def test(self, test_episodes = 1):#evaluate
        ...
        for e in range(test_episodes):
            ...
            while not done:
                ...
                if done:
                    average, SAVING = self.PlotModel(-1, score, e, save=False)
...
if __name__ == "__main__":
    ...
    agent.test(100)
    # plt.plot(agent.reward_arr)
    # plt.plot(agent.score_arr)
    # plt.show()
